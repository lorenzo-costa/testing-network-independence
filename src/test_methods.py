...
from .helper_functions.metrics_functions import rv_coefficient_adjusted
from .helper_functions.metrics_functions import first_cca_component
from .helper_functions.metrics_functions import multivariate_ac_coefficient_permutation
from ._base_class import BaseMethod, BasePermutationTest
...
class MultivariateACTest(BasePermutationTest):
    ...
    def fit(self, data, **kwargs):
        ...
        if "X" not in data.keys():
            raise ValueError("True positions X must be provided for multivariate AC test.")
...
class DiffusionCorrelation(BasePermutationTest):
