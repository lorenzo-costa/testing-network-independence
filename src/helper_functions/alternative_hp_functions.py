...
def solve_dependent(
    ...
    if fit_sigma is False:
        if not isinstance(sigma, (int, float)):
            raise Exception(
                "If fit_sigma is False, need to specify a int or float sigma"
            )
    n = A.shape[0]
...
def llk_gradient(M, A, B, sigma, n):
    ...
    theta_sigma = np.block(
        [[np.zeros((n, n)), -sigma * np.eye(n)], [-sigma * np.eye(n), np.zeros((n, n))]]
    )
...
def solver_grid(A, B, k, niters, lambda_reg=None, grid=0.1, rng=None):
    ...
    out_sigma = grid
    for sigma in grid:
        Ms, sigma_list = solve_dependent(
            A, B, k, niters, lambda_reg, fit_sigma=False, sigma=sigma, rng=rng
        )
        estimated_M = Ms[-1]
        out_M[sigma] = Ms
