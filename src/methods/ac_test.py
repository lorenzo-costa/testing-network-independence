...
class _ACMixin:
    ...
    def _adaptive_s_statistic(self, Z, Y, M):
        """Return max(xi_M(Y, Z), xi_M(-Y, Z)) for one M."""
        common = {
            "Z": Z,
            "X": self.X if not self._ignore_X else None,
            "M": M,
        ...
        positive = ac_coefficient(Y=Y, **common)
        negative = ac_coefficient(Y=-Y, **common)
...
class MultivariateACTest(_ACMixin, BasePermutationTest):
    ...
    exponents 0.1, 0.3, 0.5, 0.7, and 0.9. Adaptive mode requires univariate
    Y; all other coefficient and permutation options retain their usual
    behavior.
    """
    ...
    def _fit_adaptive_permutation(self):
        if self.Y.shape[1] != 1:
            raise ValueError("Adaptive M selection requires univariate Y.")
