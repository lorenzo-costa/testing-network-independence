...
def test_user_can_select_latent_positions_and_generate_networks(
    ...
    assert set(result) == {"A", "B", "Z", "X"}
    assert result["A"].shape == (n, n)
    assert result["B"].shape == (n, n)
    assert result["Z"].shape == (n, z_dimension)
    assert result["X"].shape == (n, x_dimension)
    ...
    np.testing.assert_allclose(result["B"], result["B"].T)
    np.testing.assert_array_equal(np.diag(result["A"]), 0)
    np.testing.assert_array_equal(np.diag(result["B"]), 0)
