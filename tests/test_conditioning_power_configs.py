...
@pytest.mark.parametrize(
    "filename, experiment_type, setup_count, design_count",
    [
        (
...
def test_conditioning_power_configs_load_and_build_complete_designs(
    ...
    assert config["simulation"]["rho"] == [0.0, 0.2]
    ...
    assert {row["rho"] for row in design} == {0.0, 0.2}
...
@pytest.mark.parametrize(
    "filename",
    ["config_conditional_copula.yaml", "config_postlinearnoise.yaml"],
)
